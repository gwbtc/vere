 ...
 */
static u3n_prog*
_n_prog_old(u3n_prog* src_u)
{
  c3_w cab_w = sizeof(u3j_site) * src_u->cal_u.len_w,
       reb_w = sizeof(u3j_rite) * src_u->reg_u.len_w,
       lib_w = sizeof(u3_noun) * src_u->lit_u.len_w,
       meb_w = sizeof(u3n_memo) * src_u->mem_u.len_w,
       pod_w = src_u->lit_u.len_w % 2,
       ped_w = src_u->mem_u.len_w % 2,
       dat_w = cab_w + reb_w + lib_w + meb_w +
               (pod_w * sizeof(u3_noun)) + (ped_w * sizeof(u3n_memo));

  u3n_prog* pog_u     = u3a_malloc(sizeof(u3n_prog) + dat_w);
  pog_u->byc_u.own_o = c3n;
  pog_u->byc_u.len_w = src_u->byc_u.len_w;
  pog_u->byc_u.ops_y = src_u->byc_u.ops_y;

  pog_u->lit_u.len_w = src_u->lit_u.len_w;
  pog_u->lit_u.non   = (u3_noun*) _n_prog_dat(pog_u);

  pog_u->mem_u.len_w = src_u->mem_u.len_w;
  pog_u->mem_u.sot_u = (u3n_memo*) (pog_u->lit_u.non + pog_u->lit_u.len_w + pod_w);

  pog_u->cal_u.len_w = src_u->cal_u.len_w;
  pog_u->cal_u.sit_u = (u3j_site*) (pog_u->mem_u.sot_u + pog_u->mem_u.len_w + ped_w);

  pog_u->reg_u.len_w = src_u->reg_u.len_w;
  pog_u->reg_u.rit_u = (u3j_rite*) (pog_u->cal_u.sit_u + pog_u->cal_u.len_w);

  memcpy(pog_u->lit_u.non, src_u->lit_u.non, dat_w);
  return pog_u;
}
...
_n_hilt_fore(u3_noun hin, u3_noun bus, u3_noun* out)
{
  ...
  switch ( tag ) {
    case c3__cash: {
      u3_atom har = u3i_word(ga_noun_map_count(u3R->cax.har_p));
      ga_noun_map_discount(u3R->cax.har_p);
      u3_atom per = u3i_word(ga_noun_map_count(u3R->cax.per_p));
      ga_noun_map_discount(u3R->cax.per_p);
...
_n_hilt_hind(u3_noun tok, u3_noun pro)
{
  ...
  if ( (c3y == u3r_cell(tok, &p_tok, &q_tok)) && (c3__loop == p_tok) ) {
    u3_noun del = u3h_del(u3R->lop_p, q_tok);
    if ( u3_none != del ) u3z(del);
  }
...
*/
static void
_cn_merge_prog_cb(u3h_item* ite_u, void* wit)
{
  u3p(u3h_root) har_p = *(u3p(u3h_root)*)wit;
  u3_weak         got;
  u3_noun         key   = ite_u->key;
  u3n_prog*       pog_u = _cn_to_prog(ite_u->val);
  got   = u3h_git(har_p, key);

  if ( u3_none != got ) {
    u3n_prog* dst_u = _cn_to_prog(got);
    _cn_merge_prog_dat(dst_u, pog_u);
    u3a_free(pog_u);
    pog_u = dst_u;
  }
 ...
 */
static u3n_prog*
_n_prog_take_merge(u3n_prog* dst_u, u3n_prog* src_u)
{
  c3_o own_o = src_u->byc_u.own_o;
  c3_w cab_w = sizeof(u3j_site) * src_u->cal_u.len_w,
       reb_w = sizeof(u3j_rite) * src_u->reg_u.len_w,
       lib_w = sizeof(u3_noun) * src_u->lit_u.len_w,
       meb_w = sizeof(u3n_memo) * src_u->mem_u.len_w,
       pad_w = (8 - src_u->byc_u.len_w % 8) % 8,
       pod_w = src_u->lit_u.len_w % 2,
       ped_w = src_u->mem_u.len_w % 2,
       dat_w = ((c3n == own_o) ? 0 : src_u->byc_u.len_w + pad_w) + 
               cab_w + reb_w + lib_w + meb_w +
               (pod_w * sizeof(u3_noun)) + (ped_w * sizeof(u3n_memo));

  dst_u     = u3a_realloc(dst_u, sizeof(u3n_prog) + dat_w);
  dst_u->byc_u.own_o = own_o;
  dst_u->byc_u.len_w = src_u->byc_u.len_w;
  if ( c3y == own_o ) {
    dst_u->byc_u.ops_y = (c3_y*) _n_prog_dat(dst_u);
    memcpy(dst_u->byc_u.ops_y, src_u->byc_u.ops_y, src_u->byc_u.len_w + pad_w);
  } else {
    dst_u->byc_u.ops_y = src_u->byc_u.ops_y;
  }

  dst_u->lit_u.len_w = src_u->lit_u.len_w;
  if ( c3y == own_o ) {
    dst_u->lit_u.non   = (u3_noun*) (dst_u->byc_u.ops_y + dst_u->byc_u.len_w + pad_w);
  } else {
    dst_u->lit_u.non   = (u3_noun*) _n_prog_dat(dst_u);
    memcpy(dst_u->lit_u.non, src_u->lit_u.non, dat_w);
  }

  dst_u->mem_u.len_w = src_u->mem_u.len_w;
  dst_u->mem_u.sot_u = (u3n_memo*) (dst_u->lit_u.non + dst_u->lit_u.len_w + pod_w);

  dst_u->cal_u.len_w = src_u->cal_u.len_w;
  dst_u->cal_u.sit_u = (u3j_site*) (dst_u->mem_u.sot_u + dst_u->mem_u.len_w + ped_w);

  dst_u->reg_u.len_w = src_u->reg_u.len_w;
  dst_u->reg_u.rit_u = (u3j_rite*) (dst_u->cal_u.sit_u + dst_u->cal_u.len_w);
...
*/
static u3n_prog*
_cn_take_merge_prog_dat(u3n_prog* dst_u, u3n_prog* src_u)
{
  c3_w i_w;
  ...
  dst_u = _n_prog_take_merge(dst_u, src_u);
  ...
  for ( i_w = 0; i_w < src_u->mem_u.len_w; ++i_w ) {
    ...
    u3z(emo_u->key);
    emo_u->sip_l    = ome_u->sip_l;
    emo_u->key      = u3a_take(ome_u->key);
    emo_u->cid      = ome_u->cid;
  }

  for ( i_w = 0; i_w < src_u->cal_u.len_w; ++i_w ) {
    u3j_site_take_merge(&(dst_u->cal_u.sit_u[i_w]),
                   &(src_u->cal_u.sit_u[i_w]));
  }

  for ( i_w = 0; i_w < src_u->reg_u.len_w; ++i_w ) {
    u3j_rite_take_merge(&(dst_u->reg_u.rit_u[i_w]),
                   &(src_u->reg_u.rit_u[i_w]));
  }
...
*/
static void
_cn_take_merge_prog_cb(u3h_item* ite_u, void* wit)
{
  u3p(u3h_root) har_p = *(u3p(u3h_root)*)wit;
  u3_noun         key   = u3a_take(ite_u->key);
  ite_u->val            = _cn_take_prog_cb(ite_u->val);

  //u3n_prog*       src_u = _cn_to_prog(ite_u->val);

  u3h_item* eti_u = u3h_put_get_ptr(har_p, key);

  //if ( u3_none == eti_u->key ) {
  //  eti_u->key = key;
  //  val = _cn_take_prog_cb(ite_u->val);
  //} else {
  //  u3z(eti_u->key);
  //  eti_u->key = key;
  //  u3n_prog* dst_u = _cn_to_prog(eti_u->val);
  //  dst_u = _cn_take_merge_prog_dat(dst_u, src_u);
  //  val = _cn_of_prog(dst_u);
  //}
  //eti_u->val = val;
  ...
  if ( u3_none == eti_u->key ) {
    ...
    u3n_prog* src_u = _cn_to_prog(ite_u->val);
    u3n_prog* dst_u = _cn_to_prog(eti_u->val);
    _cn_merge_prog_dat(dst_u, src_u);
    u3a_free(src_u);
    eti_u->val = _cn_of_prog(dst_u);
  }
}
...
{
  u3h_walk_with(har_p, _cn_take_merge_prog_cb, &u3R->byc.har_p);
 ...
 */
static void
_n_bam(u3h_item* ite_u, void* dat)
{
  u3n_prog* pog = _cn_to_prog(ite_u->val);
  c3_w*   bam_w = dat;
 ...
 */
static void
_n_feb(u3h_item* ite_u)
{
  _cn_prog_free(_cn_to_prog(ite_u->val));
}
