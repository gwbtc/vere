...
{
  u3j_rite crs_u;
  u3j_rite_take(&crs_u, src_u);
  u3j_rite_merge(dst_u, &crs_u);
...
u3j_site_take_merge(u3j_site* dst_u, u3j_site* src_u) {
  u3j_site crs_u;
  u3j_site_take(&crs_u, src_u);
  u3j_site_merge(dst_u, &crs_u);
...
*/
static void
_cj_merge_hank_cb(u3h_item* ite_u, void* wit)
{
  u3p(u3h_root) han_p = *(u3p(u3h_root)*)wit;
  u3_noun key         = ite_u->key;
  u3j_hank* nah_u     = _cj_to_hank(ite_u->val);
...
static void
_cj_take_merge_hank_cb(u3h_item* ite_u, void* wit)
{
  u3p(u3h_root) han_p = *(u3p(u3h_root)*)wit;
  ...
  if ( u3_none == nah_u->hax ) {
    ...
    u3j_hank* han_u;
    u3h_item* eti_u = u3h_put_get_ptr(han_p, key);
    if ( u3_none == eti_u->key )  {
      eti_u->key = key;
      eti_u->val = _cj_take_hank_cb(ite_u->val);
    }
    else {
      u3z(eti_u->key);
      eti_u->key = key;
      han_u = _cj_to_hank(eti_u->val);
      ...
      u3j_site_take_merge(&(han_u->sit_u), &(nah_u->sit_u));
    }
  }
}
...
u3j_reap(u3a_jets jed_u)
{
  ...
  u3h_walk_with(jed_u.han_p, _cj_take_merge_hank_cb, &u3R->jed.han_p);
...
*/
static void
_cj_warm_tap(u3h_item* ite_u, void* wit)
{
  u3_noun* rel = wit;
  *rel = u3nc(u3h_item_to_slot(ite_u), *rel);
}

/* _cj_ream_hank(): clear hot state out of hook sites.
*/
static void
_cj_ream_hank(u3h_item* ite_u)
{
  u3j_site_ream(&(_cj_to_hank(ite_u->val)->sit_u));
}
...
*/
static void
_cj_mark_hank(u3h_item* ite_u, void* dat)
{
  c3_w* tot_w = (c3_w*) dat;
  u3j_hank* han_u = _cj_to_hank(ite_u->val);
